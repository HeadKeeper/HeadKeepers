...
import com.headkeeper.bean.view.document_gen.UserListView;
...
public class Users extends Document {
    ...
    private UserListView userListDTO;

    @Override
    public void setObjectList(Object object) {
        this.userListDTO = (UserListView) object;
    }
    ...
    public void buildPdf(com.itextpdf.text.Document doc) throws DocumentException {
        try {
            ...
            ArrayList<ShortUserInformationView> users = this.userListDTO.getUsers();
            ...
            for (ShortUserInformationView user : users) {
                ...
                table.addCell(new PdfPCell(new Paragraph(user.getNickname(), font)));
                table.addCell(new PdfPCell(new Paragraph(user.getRole(), font)));
            }
            doc.add(table);
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public void buildXls(Workbook workbook) throws Exception {
        ...
        for (ShortUserInformationView user: users) {
            ...
            commentRow.createCell(2).setCellValue(user.getNickname());
            commentRow.getCell(2).setCellStyle(otherCellStyle);
            commentRow.createCell(3).setCellValue(user.getRole());
    ...
    public void buildCsv(ICsvBeanWriter writer) throws IOException   {
        ...
        ArrayList<ShortUserInformationView> users = this.userListDTO.getUsers();
        for (ShortUserInformationView user : users) {
            writer.write(new UserBean(String.valueOf(user.getId()), user.getEmail(), user.getNickname(), user.getRole()), header);
        }
    }
